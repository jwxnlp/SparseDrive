...
for idx in tqdm(range(len(data_infos))):
    info = data_infos[idx]
    plan_traj = info['gt_ego_fut_trajs'].cumsum(axis=-2) # [N_t=6, 2]
...
for trajs in navi_trajs:
    ...
    cluster = KMeans(n_clusters=K).fit(trajs).cluster_centers_ # [K, 12]
    cluster = cluster.reshape(-1, 6, 2) # [K, 6, 2]
...
clusters = np.stack(clusters, axis=0) # [N_c, K, 6, 2]
